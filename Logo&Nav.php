<!DOCTYPE html>
<html>
        <head>
            <title>Webshop Ronan&Co</title>
            <meta charset="utf-8">
            <link rel="stylesheet" href="/webdesign/Webshop/Logo&Nav.css" type="text/css">
            
        </head>

        <body>
            <div class="header">
                <img src="/webdesign/Webshop/logo_small.png" 
                    width="250"
                    height="100"/>
                <p> <b>Software & Hardware</b></p>
            </div>

            <div class="categories">
                <a href="/webdesign/Webshop/Webshop.php"><b>Home</b></a>
                <a href="/webdesign/Webshop/category.php/?category=1"><b>Laptops</b></a>
                <a href="/webdesign/Webshop/category.php/?category=2"><b>Software</b></a>
                <a href="/webdesign/Webshop/category.php/?category=3"><b>Graphics Cards</b></a>
                <a href="#"><b>Category 4</b></a>
                <a href="#"><b>Category 5</b></a>
                <a href="#" class="right">My Account</a>
            </div>
        </body>
</html>